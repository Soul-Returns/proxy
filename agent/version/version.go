...
// Current is the current version of the agent (set via ldflags during build)
var Current = "1.0.0"
...
// UpdateChannel represents the update channel preference
type UpdateChannel string

const (
	ChannelRelease    UpdateChannel = "release"
	ChannelPreRelease UpdateChannel = "pre-release"
)
...
func CheckForUpdates(channel UpdateChannel) (*UpdateInfo, error) {
	...
	req.Header.Set("User-Agent", "DevProxy-Agent/"+Current)
	...
	for i := range releases {
		...
		if channel == ChannelRelease && r.Prerelease {
			continue
		}
